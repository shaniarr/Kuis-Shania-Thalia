...
def get_test_data_acdc(src_path, patient, gt_available=False):
    ...
    list_of_files = sorted( filter( lambda x: os.path.isfile(os.path.join(folder_path, x)),
                            os.listdir(folder_path) ) )
    print(patient_data)
    # Read patient Number
    m = re.match("patient(\d{3})", patient)
    patient_data['ED'] = list_of_files[1][-8:-7]
    patient_data['ES'] = list_of_files[2][-9:-7]
    if patient_data['ES'][0] == '0':
        patient_data['ES'] = patient_data['ES'][1]
        print("anjay")
    print(patient_data)
    # patient_No = int(m.group(1))
    # Read Diastole frame Number
    # ED_frame_No = int(patient_data['ED'])
    ed_img = list_of_files[1]
    print(ed_img)
    ed, affine, hdr  = load_nii(ed_img, folder_path)
    # Read Systole frame Number
    # ES_frame_No = int(patient_data['ES'])
    es_img = list_of_files[2]
    print(es_img)
    es, _, _  = load_nii(es_img, folder_path)

    img_4d = list_of_files[0]
